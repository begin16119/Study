...
public class ExprParse {
    ...
    public List<MyToken> prase(String expr){

        List<MyToken> tokens = new LinkedList<>();
        ...
        while(i < expr.length()){
            ...
            if(isOperator(c)){
                ...
                MyToken t = new MyToken(MyToken.OPERATOR,String.valueOf(c));
                tokens.add(t);
                i++;
            }else if(isNumber(c)){
                int nextIsNumberIndex = nextIsNumber(expr,i);
                String num = expr.substring(i,nextIsNumberIndex);
                MyToken t = new MyToken(MyToken.NUMBER,num);
                tokens.add(t);
                i=nextIsNumberIndex;
            }else{
